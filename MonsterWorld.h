...
#include "NHS_실습11_VariousMonsters.h"
#include "NHS_실습11_Human.h"
#include "NHS_실습11_Matrix.h"
#include <windows.h>
#define MAXMONS 8
...
public:
	...
	void play(int maxwalk, int wait) {													// 실제적인 게임을 실행시키는 함수
		...
		cerr << " 엔터를 누르세요...";
		getchar();
		for (int i = 0; i < maxwalk; i++) {												// 각 몬스터별 move()가 가상 함수 이므로 강제 형 변환 없이도 실행시간에 생성되었던 클래스를 찾고, 해당 move() 실행 가능
